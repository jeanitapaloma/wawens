...
return defaultOpen;}
$.fn.collapsible.defaults={cssClose:'collapse-close',cssOpen:'collapse-open',cookieName:'collapsible',cookieOptions:{path:'/',expires:7,domain:'',secure:''},defaultOpen:'',speed:'slow',bind:'click',animateOpen:function(elem,opts){elem.next().stop(true,true).slideDown(opts.speed);},animateClose:function(elem,opts){elem.next().stop(true,true).slideUp(opts.speed);},loadOpen:function(elem,opts){elem.next().show();},loadClose:function(elem,opts){elem.next().hide();}};})(jQuery);var collapsHeight=0;jQuery(document).ready(function($){if($(window).width()>640){$(".page_collapsible").on("click",function(){var wrapHeight=$(this).next('.wcfm-container').height();var wrapOHeight=$(this).next('.wcfm-container').outerHeight();if(wrapHeight!=1){if(collapsHeight>wrapHeight){$('.wcfm-tabWrap').css('height',(collapsHeight+100));$(document.body).trigger('wcfm_product_tabwrap_changed',(collapsHeight+100));}else{$('.wcfm-tabWrap').css('height',(wrapHeight+100));$(document.body).trigger('wcfm_product_tabwrap_changed',(wrapHeight+100));}}});}});function resetCollapsHeight(multi_input_holder){if(jQuery(window).width()>640){if(multi_input_holder.attr('id')!='gallery_img'){if(multi_input_holder.hasClass('wcfm_admin_shipping_setting_block'))
multi_input_holder=multi_input_holder.parent().parent().parent();var wrapHeight=multi_input_holder.parent().height();if(wrapHeight!=null&&wrapHeight!=1){if(collapsHeight>wrapHeight){jQuery('.wcfm-tabWrap').css('height',(collapsHeight+150));jQuery(document.body).trigger('wcfm_product_tabwrap_changed',(collapsHeight+150));}else{if(multi_input_holder.parent().hasClass('shipping_type')){jQuery('.wcfm-tabWrap').css('height',(wrapHeight+300));jQuery(document.body).trigger('wcfm_product_tabwrap_changed',(wrapHeight+300));}else{jQuery('.wcfm-tabWrap').css('height',(wrapHeight+150));jQuery(document.body).trigger('wcfm_product_tabwrap_changed',(wrapHeight+150));}}}}}}